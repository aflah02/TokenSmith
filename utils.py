...
class WriteableMMapIndexedDataset:
    def __init__(self, dataset_prefix: str, batch_info_prefix: str, train_seq_len: int = 1024, add_extra_token_to_seq : int = 1):
        logger.debug(f"Initializing WriteableMMapIndexedDataset with pointer: {dataset_prefix}.bin and index: {dataset_prefix}.idx")
        logger.debug(f"Loading doc/sample/shuffle indexes with prefix: {batch_info_prefix}")
        ...
        if not os.path.exists(f"{batch_info_prefix}_doc_idx.npy"):
            self.simulate_training_run(batch_info_prefix)
        self.batch_info = BatchInfo(batch_info_prefix)
    ...
    def simulate_training_run(self, batch_info_prefix: str):
        """
        ...
        logger.warning(f"> Data prefix: {data_prefix}, num_samples: {num_samples}, seq_length: {seq_length}, seed: {seed}, packing_impl: {packing_impl}")
    ...
    def write_example_into_corpus(self, injection_loc: int, injection_data: np.ndarray, dry_run: bool = False):
        ...
        if doc_index_f_ == doc_index_l_:
            ...
            logger.debug(f'>>> Inserting {ex_space} of {len(injection_data)} tokens starting at position {pt_byte_offset}')
            if not dry_run:
                self.corpus_pointer.seek(pt_byte_offset)
            ...
            if ex_left > 0:
                ...
                ex_left -= ex_space
                logger.debug(f'>>> Inserting {max(ex_left, ex_space)} of {len(injection_data)} tokens starting at position {pt_byte_offset}')
                if not dry_run:
                    self.corpus_pointer.seek(pt_byte_offset)
                    self.corpus_pointer.write(injection_data[ex_done:ex_done+ex_space].tobytes(order="C"))
                ex_done += ex_space
    ...
    def inject_example_into_corpus(self, injection_loc: int, injection_data: np.ndarray,
                                   injection_type: str, rng: np.random.Generator, dry_run: bool = False):
        """
